...
export class PrintService {
  ...
  async download(contactoData: ContactCard[]): Promise<void> {
    // Cargar SVGs y convertirlos a PNG
    ...
    // Definir variables para el manejo de páginas
    const pageHeight = pdf.internal.pageSize.height;
    const marginBottom = 10;
    const defaultTopMargin = 20;

    const green = '21, 125, 53';
    const blue = '6, 114, 18';
  
    // Función auxiliar para verificar si se requiere agregar una nueva página
    const checkAddPage = (lineHeight: number) => {
      if (posY + lineHeight > pageHeight - marginBottom) {
        pdf.addPage();
        posY = defaultTopMargin;
      }
    };
    ...
    pdf.setFontSize(18);
    pdf.text(contactoData[0].title.text, marginX, posY);
  
    posY += 12;
    pdf.setFontSize(12);
  
    // --- Iterar secciones ---
    if (contactoData[0].sections) {
      contactoData[0].sections.forEach((element) => {
        pdf.setFont('Helvetica', 'normal');
  
        // Sección "text-top"
        if (element.type === 'text-top') {
          posY += 5;
          element.value.forEach(item => {
            if (item) {
              checkAddPage(7);
              posY += 7;
              pdf.text(`${item.label}: ${item.value}`, marginX, posY);
            }
          });
        }
        ...
        if (element.type === 'badges') {
          ...
          pdf.text(element.title, marginX, posY);
  
          const badgeHeight = 8;
          const badgePadding = 6; // espacio extra a los lados
          ...
          element.badges.forEach(item => {
            ...
            pdf.text(value, posX + badgePadding / 2, posY + badgeHeight - 2);
  
            posX += dynamicBadgeWidth + badgeSpacing;
          });
        }
  
        // Sección "expandible-column" o "expandible-row"
        if (element.type === 'expandible-column' || element.type === 'expandible-row') {
          posY += 15;
          pdf.setFont('Helvetica', 'bold');
          pdf.setTextColor(0, 0, 0);
          pdf.text(element.title, marginX, posY);
  ...
  //   pdf.setFont('Helvetica', 'bold');
  //   pdf.setFontSize(18);
  //   pdf.text(contactoData[0].title.text, 10, posY);
  ...
  //             // pdf.text(`${item.text}: ${item.value}`, marginX, posY);
  //             // posY += 7;
  //           // }
  //         });
  //       }
  ...
  //   pdf.save(`idcredito-prueba}`);
  // }


  async download2(contactoData: any): Promise<void> {
